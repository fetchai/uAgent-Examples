...
This agent analyzes the latest Reddit posts retrieved from the Post Extractor Agent to identify a concise and descriptive overarching topic. It ensures distinct categories, and returns the most relevant topic to the requester.
...
sequenceDiagram
    ...
    activate User
    User->>Topic: Request topic analysis
    activate Topic
    ...
    deactivate OpenAI
    Topic->>User: Respond with topic summary
    deactivate Topic
```
...
TopicAnalysisResponse(
    topic="Migration to Alternative Social Platforms",
)
```
...
TOPIC_AGENT_ADDRESS = "<deployed_agent_address>"
TOPIC = "Technology"

@agent.on_event("startup")
async def handle_startup(ctx: Context):
    """Send the prompt to the AI agent on startup."""
    await ctx.send(TOPIC_AGENT_ADDRESS, TopicAnalysisRequest(subreddit=TOPIC))


@agent.on_message(TopicAnalysisResponse)
