...
This agent is designed to send direct messages on Slack. Given a Slack user ID and a message, it delivers the message to the specified user. Note that recipients must belong to the workspace associated with the provided Slack token. To ensure this use case works, be sure to include the chat:write and im:write scopes in your Slack token.
...
SLACK_AGENT_ADDRESS = "<deployed_agent_address>"
