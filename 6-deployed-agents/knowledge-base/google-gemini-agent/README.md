...
@agent.on_message(CodeResponse)
async def handle_response(ctx: Context, sender: str, msg: CodeResponse):
    ctx.logger.info(f"Received response from {sender}:")
    ctx.logger.info(msg.text)
...
async def handle_response(ctx: Context, sender: str, msg: StructuredOutputResponse):
    ctx.logger.info(f"Received response from {sender}:")
    ctx.logger.info(Location.parse_obj(msg.output))
