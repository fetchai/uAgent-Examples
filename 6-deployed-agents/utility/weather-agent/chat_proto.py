...
from weather import get_weather, WeatherForecastRequest


AI_AGENT_ADDRESS = os.getenv("claude-ai-agent")
        ...
        if isinstance(item, StartSessionContent):
            ...
            await ctx.send(
                ...
                StructuredOutputPrompt(
                    prompt=item.text, output_schema=WeatherForecastRequest.schema()
                ),
            )
        else:
...
async def handle_structured_output_response(
    ...
    prompt = WeatherForecastRequest.parse_obj(msg.output)

    try:
        weather_forecast = await get_weather(prompt)
    ...
    if "error" in weather_forecast:
        await ctx.send(session_sender, create_text_chat(str(weather_forecast["error"])))
        return

    chat_message = create_text_chat(
        f"Location: {weather_forecast['location']}\n"
